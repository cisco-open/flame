...
class ExperimentReproducer:
    ...
    def reproduce_from_config(self, exec_config_path: Path):
        ...
        self.trainer_spawner = TrainerSpawner(
            ...
            sleep_between_spawns=config["experiment"]["execution"][
                "sleep_between_spawns"
            ],
    ...
    def _verify_metadata(self, metadata_refs: Dict):
        ...
        if trace_key.startswith("syn_"):
            trace_path = (
                self.metadata_dir / "availability_traces" / "synthetic_traces.yaml"
            )
        else:
            trace_path = (
                self.metadata_dir / "availability_traces" / "mobiperf_traces.yaml"
            )
    ...
    def _create_log_prefix(self, config: Dict) -> str:
        ...
        return (
            f"{timestamp_str}_{selector}_n{num_trainers}_{tracking}_{alpha_str}_{avail}"
        )
...
""",
    ...
    parser.add_argument("config", type=Path, help="Path to execution_config.yaml")
