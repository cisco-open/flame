...
LIFL switches to different communication backends based on use cases:
- gRPC (p2p): leaf aggregator <=> client, inter-node communication in LIFL's model aggregation service
- Shared Memory: intra-node communication in LIFL's model aggregation service
"""
...
from flame.backend.lifl.lifl_chunk_manager import LIFLChunkManager
from flame.backend.lifl.lifl_chunk_store import LIFLChunkStore
...
class LIFLSharedMemoryBackend(AbstractBackend):
    ...
    def __init__(self):
        ...
        self.chunk_mgr = LIFLChunkManager(self)
