...
def multi_column(filepath):
    ...
    try:
        ...
        sections = {cfg["train_label"]: train_time, cfg["idle_label"]: idle_time}
        ...
        print(
            f"Error processing '{filepath}' with 'multi_column':\n{traceback.format_exc()}"
        )
...
def multi_row(filepath):
    ...
    try:
        ...
        print(
            f"Error processing '{filepath}' with 'multi_row':\n{traceback.format_exc()}"
        )
...
strategies = {"multi_row": multi_row, "multi_column": multi_column}

try:
    strategy_func = strategies[PLOTTING_CONFIG["format_strategy"]]
except KeyError:
    print(
        f"Error: Invalid 'format_strategy' in PLOTTING_CONFIG. Choose from {list(strategies.keys())}"
    )
    exit()

print(
    f"Starting data processing using '{PLOTTING_CONFIG['format_strategy']}' strategy..."
)
