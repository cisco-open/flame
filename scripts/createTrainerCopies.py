...
def main():
    ...
    if END_TRAINER_NUM <= start_trainer_num:
        print(f"Error: END_TRAINER_NUM ({END_TRAINER_NUM}) must be greater than start trainer number ({start_trainer_num})")
        return
    ...
    try:
        ...
        if len(original_taskid) < 4:
            print(f"Error: taskid is too short (less than 4 characters). taskid='{original_taskid}'")
            return
        ...
        print(f"Error: Could not parse taskid. taskid='{src_data.get('taskid', 'NOT FOUND')}', error={e}")
        return
