...
export const getErrorMessages = (schemaValidity: SchemaValidity) => {
  ...
  if (schemaValidity.duplicateNames?.length) {
    errorMessages.push(`${schemaValidity.duplicateNames.join(', ')} ${schemaValidity.duplicateNames.length === 1 ? 'is' : 'are'} duplicate`)
  }
...
export const createCodeFileData = (fileData: any, data: any) => {
  let newFiles: any[];
  const index = fileData.findIndex((file: any) => file.roleName === data.roleName);
  if (index === -1) {
    newFiles = [...fileData, data];
  } else {
